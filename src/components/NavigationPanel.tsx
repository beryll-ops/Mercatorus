...
export const NavigationPanel: React.FC<NavigationPanelProps> = ({
  ...
  return (
    <div className="bg-slate-900/95 border border-slate-700/80 backdrop-blur-xl rounded-2xl p-3 sm:p-4 shadow-2xl space-y-2 sm:space-y-2.5">
        ...
        <div className="space-y-1.5 sm:space-y-2">
          ...
          <div className="grid grid-cols-2 p-1 bg-slate-800/90 rounded-xl border border-slate-700/60 text-xs font-semibold">
            <button
              ...
              className={`flex items-center justify-center gap-1.5 py-2 px-2.5 sm:px-3 rounded-lg transition min-h-[44px] cursor-pointer ${
                travelMode === 'walking'
          ...
          {/* Stats Display */}
          <div className="flex items-center justify-between px-2.5 py-1.5 sm:px-3 sm:py-2 bg-slate-800/80 rounded-xl border border-slate-700/50">
      ...
      {!distanceResult && (
        <div className="flex items-center gap-2 px-2.5 py-1.5 bg-slate-800/60 rounded-xl text-xs text-slate-300 border border-slate-700/40">
          <MapPin className="w-3.5 h-3.5 text-sky-400 shrink-0" />
          <span className="truncate">Cel: środek terenu ({territoryName})</span>
        </div>
      )}
      ...
      <a
        ...
        className="w-full min-h-[44px] sm:min-h-[48px] px-3.5 py-2.5 sm:px-4 sm:py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2 transition active:scale-[0.98] border border-blue-400/30 text-xs sm:text-sm md:text-base cursor-pointer"
        title="Otwórz nawigację krok po kroku w aplikacji Google Maps"
      >
        <Navigation2 className="w-4 h-4 sm:w-5 sm:h-5 text-white shrink-0" />
        <span className="truncate">
          {travelMode === 'walking' ? 'Nawiguj pieszo w Google Maps' : 'Nawiguj autem w Google Maps'}
        </span>
        <ExternalLink className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-200 ml-auto shrink-0" />
      </a>
    </div>
  );
};
